...
import { listMeasurementSessionIndex } from '../../../../lib/sessionIndexReader';
...
export const Y3MeasurementHistory: React.FC = () => {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        const items = await listMeasurementSessionIndex(user.uid);
        ...
        console.error('Failed to load measurement history:', e);
  ...
  const download = async (row: HistoryRow) => {
    ...
    try {
      ...
      const bytes = await buildMeasurementPdf({
        ...
        studentName: getActiveStudentName() ?? 'Student',
        score,
        sessionId: row.sessionId,
      });
      ...
      console.error('Failed to download measurement session:', e);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-gray-900 font-bold text-xl">Measurement History</div>
          <div className="text-gray-600 text-sm mt-1">Saved practice workbooks</div>
        </div>
        <div className="flex flex-col items-end gap-3">
          <button
            type="button"
            onClick={() => navigate('/y3/numeracy/measurement')}
      ...
      {loading ? (
        ...
        <div className="mt-4 text-gray-600">{rows.length === 0 ? 'No measurement sessions yet.' : 'No sessions in the selected date range.'}</div>
      ) : (
              ...
              {sorted.map((r) => {
                ...
                return (
                  <tr key={r.sessionId} className="border-t border-gray-100">
                    ...
                    <td className="py-2 pr-4 text-gray-700">{typeof r.percentage === 'number' ? `${r.percentage}%` : '-'}</td>
                    <td className="py-2">
