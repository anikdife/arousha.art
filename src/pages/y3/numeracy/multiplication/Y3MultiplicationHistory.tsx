...
export const Y3MultiplicationHistory: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const items = await listTopicSessionIndex(uidToLoad, 'multiplication');
        if (items.length > 0) {
        ...
        const mapped: SessionIndexItem[] = stored
          .filter((s) => String(s.sessionId).startsWith('multiplication-'))
          .map((s) => {
        ...
        console.error('Failed to load multiplication history index:', e);
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      {!loading && !error && dateFilteredSessions.length === 0 && (
        <div className="mt-6 text-gray-600">No multiplication practice sessions found for this date range.</div>
      )}
        ...
        <div className="mt-6 space-y-3">
          {dateFilteredSessions.map((item) => {
            ...
            return (
              ...
              >
                <div>
                  <div className="text-gray-900 font-semibold">Multiplication practice</div>
